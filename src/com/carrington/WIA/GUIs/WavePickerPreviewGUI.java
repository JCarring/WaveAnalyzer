...
public class WavePickerPreviewGUI extends JDialog implements PFPickListener, WavePickListener {
	...
	public static void main(String[] args) {
		...
		final SavGolSettings defSav = Savgol.generateSettings(101,  3);
		final boolean defSavEnabled = false;
		final boolean defAllowAlignWrap = true;
		final boolean defAllowAlignWrapExcessDisc = false;
		SavGolSettings currSav = Savgol.generateSettings(101,  3);
		boolean currSavEnabled = false;
		...
		HemoData hd1 = data.getData().copy();
		HemoData hd2 = data.getData().copy();
		HemoData hd3 = data.getData().copy();
		hd1.setName("test1");
		hd2.setName("test2");
		hd3.setName("test3");
		...
		while (status == WavePickerPreviewGUI.PREVIEW_NEXT || status == WavePickerPreviewGUI.PREVIEW_LAST) {
			...
			WavePickerPreviewGUI wavepickerGUI = new WavePickerPreviewGUI(beatCopy.getName(), beatCopy, pr, hasPrevious, hasNext, currSav, currSavEnabled,
					currAllowAlignWrap, currAllowAlignWrapExcessDisc, maintain, null);
			...
			case WavePickerPreviewGUI.PREVIEW_NEXT:
				currSelectionIndex++;
				currSelectionIndex = Math.min(currSelectionIndex, beats.size() - 1); // safe guard to not go past last index, but shouldn't happen
				break;
			case WavePickerPreviewGUI.PREVIEW_LAST:
				currSelectionIndex--;
				currSelectionIndex = Math.max(currSelectionIndex, 0); // safe guard to not go to negative index, but shouldn't happen
				break;

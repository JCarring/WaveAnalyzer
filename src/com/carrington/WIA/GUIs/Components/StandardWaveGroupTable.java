...
public class StandardWaveGroupTable extends JTable{
	...
	private StandardWaveGroupTable(String[] cols, StandardWaveGroupTableListener listener) {
		...
		int[] columnWidths = new int[] { Utils.getFontParams(cellFont, cols[0])[1] * 2,
				...
				(int) (Utils.getFontParams(cellFont, cols[2])[1] * 1.5),
		...
		Action info = new AbstractAction()
		{
			...
			public void actionPerformed(ActionEvent e)
			{
				...
				JCPopupInfoList.displayPopup("Waves included in \"" + waveGroup.getName() + "\":", waveGroup.getStandardWaves().toArray(new StandardWave[0]), ref.get());
		...
		getModel().addTableModelListener(new TableModelListener() {
			...
			public void tableChanged(TableModelEvent e) {
				if (e.getType() == TableModelEvent.UPDATE) {
			        ...
			        if (currRow == -1) {
				        Utils.showError("This group already exists!", ref.get());
	...
	public void addGroups(WIAStats wiastats) {
		...
		MyTableModel model = (MyTableModel) getModel();
		for (StandardWaveGrouping group : this.stats.getWaveGrouping()) {
			model.addRow(new Object[] { group.getName(), group.getStandardWavesString(), Utils.IconQuestion, Utils.IconFail });
